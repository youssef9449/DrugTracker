...
1. **Generate a release keystore** (one-time, save it forever — losing it means you can never publish an update to the same app).
   ...
   ```bash
   keytool -genkeypair \
     -keystore naghnagh-release.keystore \
     -alias naghnagh \
...
2. **Configure `android/app/build.gradle`** to use the keystore for the release build. Open `android/app/build.gradle` and inside the `android { ... }` block, add:
   ...
   signingConfigs {
       release {
           storeFile file('<absolute-path-to>/naghnagh-release.keystore')
...
3. **Set the Android SDK location** — create `android/local.properties` with the path to your Android SDK:
   ...
   ```
   This file is gitignored by default. Environments that set `ANDROID_HOME` automatically (e.g. CI) don't need it, but a local build via `./gradlew` requires it.
...
Assume you are at the repo root and the keystore (`naghnagh-release.keystore`)
...
```gradle
def keystorePath = file("${rootProject.projectDir}/../naghnagh-release.keystore")
...
3. **Verify the bump + signature**:
   ...
   # Expect: d3b89977…9506088  (SAME as the previous release's key)
   ```
...
1. **Deploy the web build to a public URL**. Any static host works (Vercel, Netlify, GitHub Pages, Cloudflare Pages). For Vercel:
   ...
   vercel --prod
   ```
   You'll get a URL like `https://naghnagh.vercel.app`.
...
- Use the capsule icon from `public/assets/icons/icon.svg` / `icon-512.png` as the launcher icon.
