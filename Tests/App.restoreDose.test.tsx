...
async function takeDoseViaCard(doseId: string): Promise<void> {
  ...
  await waitFor(() => {
    expect(
      requireDefined(readPersistedMedications()[0], 'readPersistedMedications()[0]')
        .doseConsumptionHistory?.[doseId]
    ).toContain(getTodayDateString());
  });
}
...
describe('App — Take → Restore → Restore blocked (explicit doseId via card)', () => {
  it('first Restore of d1 succeeds; second Restore of same d1+date is blocked', async () => {
    ...
    await waitFor(() => {
      ...
      expect(requireDefined(med, 'med').doseSkippedHistory?.d3).toBeUndefined();
      expect(requireDefined(med, 'med').doseConsumptionHistory?.d2).toBeUndefined();
      expect(requireDefined(med, 'med').doseConsumptionHistory?.d3).toBeUndefined();
      expect(requireDefined(med, 'med').currentPills).toBe(pillsAfterTake + 1);
...
describe('App — independent multi-dose Restore via SelectDoseModal', () => {
  it('restores only d2 when both d1 and d2 are already manually consumed', async () => {
    ...
    await waitFor(() => {
      const med = readPersistedMedications()[0];
      // d1 untouched
      expect(requireDefined(med, 'med').doseConsumptionHistory?.d1).toEqual([TEST_DATE]);
      expect(readLogs().filter((l) => l.type === 'skipped_day' && l.doseId === 'd1')).toHaveLength(
        0
      );
      // d2 restored only
      expect(requireDefined(med, 'med').doseConsumptionHistory?.d2).toBeUndefined();
      expect(requireDefined(med, 'med').doseSkippedHistory?.d2).toEqual([TEST_DATE]);
...
describe('App — Take → Restore → Take → Restore for the SAME doseId (card path)', () => {
  it('explicit d1 Take → Restore → Take → Restore is allowed', async () => {
    ...
    await waitFor(() => {
      const med = readPersistedMedications()[0];
      expect(requireDefined(med, 'med').doseConsumptionHistory?.d1).toContain(getTodayDateString());
