...
describe('PharmacyShoppingView — refill actions', () => {
  ...
  it('keeps the period quantity unchanged when switching display units', () => {
    ...
    // Period quantity is normalized to whole packages (describeOrderInBoxes):
    // 30 pills = 3 strips × 10 = exactly ONE full box, so the period chip and
    // the period total both render "علبة واحدة".
    expect(screen.getAllByText('علبة واحدة').length).toBeGreaterThanOrEqual(1);
    // The "(30 قرصاً)" pill-count suffix is added only in custom-quantity
    // mode (PharmacyShoppingMedicationRow).
    expect(screen.getByText('الإجمالي: علبة واحدة')).toBeInTheDocument();

    // Display-unit switching lives in custom-quantity mode. Switching the
    // unit between strips and boxes keeps the requested pills unchanged:
    // 3 strips × 10 pills = 1 box × 30 pills = the same 30-pill quantity.
