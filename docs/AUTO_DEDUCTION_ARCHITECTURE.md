...
- **Receiver separation (security):**
  ...
  - `DrugTrackerAlarmSystemReceiver` — the single shared system lifecycle receiver for `BOOT_COMPLETED`, `QUICKBOOT_POWERON`, `TIMEZONE_CHANGED`, and `SCHEDULE_EXACT_ALARM_PERMISSION_STATE_CHANGED`, `android:exported="true"`. `ExactAlarmLifecycle` performs the single exact-alarm capability check and dispatches that result through the registered `AutoDeductionAlarmFeature`, `CriticalStockAlarmFeature`, and `DoseReminderAlarmFeature` adapters. Critical Stock and Dose Reminder remain notification-plugin-owned in this phase, so their adapters are lifecycle integration points rather than duplicate schedulers/receivers.
