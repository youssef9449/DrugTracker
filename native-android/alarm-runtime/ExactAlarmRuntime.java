...
public final class ExactAlarmRuntime {
    ...
    /**
     * Atomically claim the current one-shot for delivery. This is the
     * delivery linearization point: cancellation/replacement that wins the
     * shared lock first makes this return false.
     */
    public boolean beginOneShotDelivery(
            String storageKey,
            String expectedOperationVersion) {
        if (storageKey == null || storageKey.isEmpty()
                || expectedOperationVersion == null
        ...
        synchronized (ExactAlarmOperationLock.LOCK) {
            ...
            try {
                ...
                String state = metadata.optString("deliveryState", "");
                if ("accepted".equals(state) || "in_progress".equals(state)) {
                    return false;
                }
                metadata.put("deliveryState", "in_progress");
    ...
    /**
     * Re-open a failed delivery attempt without changing operation ownership.
     */
    public boolean clearOneShotDeliveryInProgress(
            String storageKey,
            String expectedOperationVersion) {
        if (storageKey == null || storageKey.isEmpty()
                || expectedOperationVersion == null
        ...
        synchronized (ExactAlarmOperationLock.LOCK) {
            ...
            try {
                ...
                if (!"in_progress".equals(
                        metadata.optString("deliveryState", ""))) {
                    return true;
                }
                metadata.remove("deliveryState");
                return store.writeScheduleRawLocked(
                        storageKey,
                        metadata.toString());
            } catch (JSONException | RuntimeException e) {
