...
export const AndroidFab: FC<AndroidFabProps> = ({ onClick }) => {
  ...
  return (
    <div className="absolute bottom-[100px] left-4 z-40 pointer-events-auto">
      <button
        type="button"
        onClick={onClick}
        id="add-medicine-fab"
        aria-label="إضافة دواء جديد"
        className="h-14 px-5 bg-teal-700 hover:bg-teal-800 active:bg-teal-900 active:scale-98 text-white font-medium text-sm rounded-2xl shadow-md hover:shadow-lg transition-all duration-200 flex items-center gap-2.5 cursor-pointer focus:outline-none focus-visible:ring-3 focus-visible:ring-teal-500/40 select-none"
      >
        <Plus className="w-5 h-5 stroke-[2.25]" aria-hidden="true" />
